<div class="modal fade pr-0" id="{{ $modalId }}" tabindex="-1" role="dialog" data-backdrop="static" data-keyboard="false">
    <div class="modal-dialog modal-dialog-centered modal-xl m-w-100">
                ...
                <div class="row preview-bar py-3 align-items-center">
                    ...
                    <div class="col-12 col-lg-4 text-lg-right">
                        ...
                        <a href="#" class='btn btn--redoutline btn--lightborder btn--lightborder--ft rounded-pill mb-1 assistant f-16'
                            ...
                            <span v-if="! forms[0].busy"><i class="fas fa-check"></i> {{ __('Save Changes')}}</span>
                                                ...
                                                <div class="d-flex progressbar-container progressbar-container--thick mt-auto">
                                                    <div class="w-85">
                                                        <div class="d-flex justify-content-between mb-1">
                                                            <div class="aleo f-20" v-if="campaign.fundraising_goal">
                                                        ...
                                                        <div class="d-flex" v-if="campaign.total_donations">
                                                            <div class="text-grey-5 f-14 font-weight-bold sub__text mt-3 mt-sm-1">
                                                                    {{ __('Raised by') }} @{{ campaign.total_donations }} {{ __('donors') }} <div class="d-inline font-weight-normal" v-if="campaign.published_at">{{ __('since') }} @{{ $root.convertUTCToBrowser(campaign.published_at, 'M/D/YY') }}</div></div>
                                                        </div>
                                                    </div>
                                                </div>
                ...
                <div class="container">
                    <div class="row">
                        ...
                        <div class="col-md-4 pl-lg-5">
                            <div v-if="forms[0].title">
                                <h3 class="aleo">{{ __('Rewards') }}</h3>
                                <div v-for="(reward, index) in forms">
                                    <div class="d-md-flex d-none" v-if="rewards[index].image && reward.title">
                                        <img :src="rewards[index].image" alt="Rewards">
                                    </div>

                                    <div class="rewards-card p-4 mb-3" v-if="reward.title">
                                        <div class="float-right d-block d-md-none" v-if="rewards[index].image && reward.title">
                                            <img class="m-w-120" :src="rewards[index].image" alt="Rewards">
                                        </div>
                                        <div class="adjust-amount" v-if="reward.min_amount">
                                            <sup>@{{ organization.currency.symbol }}</sup> <span class="rewards-card__amount rewards-card__amount--fs-40">@{{ reward.min_amount }}</span>
                                        </div>
                                        <div class="">
                                            <div class="rewards-card__title f-18 assistant break-word" v-if="reward.title">@{{ reward.title }}</div>
                            ...
                            <div class="d-block d-md-none">
                                ...
                                <div class="organizer d-flex flex-wrap my-5">
                                    <h3 class="aleo w-100">{{ __('Organizer') }}</h3>
                                    <div class="mr-4">
                                        <img :src="organization.logo" alt="organization.name" v-if="organization.logo" class="btn-file--small rounded-circle float-left">
                                    </div>
                                    <address>
                                        <h4 class="mb-0">@{{ organization.name }}</h4>
                                        @{{ organization.address1 }}<br>
